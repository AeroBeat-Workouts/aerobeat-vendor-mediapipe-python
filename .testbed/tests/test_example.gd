...
func test_addons_manifest_symlinks_camera_tracking_for_repo_local_validation() -> void:
	...
	assert_true(block_start >= 0, "addons manifest should include aerobeat-tool-camera-tracking for backend validation")
	...
	assert_true(camera_tracking_block.contains('"url": "../../aerobeat-tool-camera-tracking"'), "addons manifest should use the repo-local camera-tracking path for validation")
	assert_true(camera_tracking_block.contains('"source": "symlink"'), "addons manifest should symlink the repo-local camera-tracking dependency")
	assert_false(camera_tracking_block.contains('"git@github.com:AeroBeat-Workouts/aerobeat-tool-camera-tracking.git"'), "addons manifest should not point camera tracking at an SSH git remote")
	assert_false(camera_tracking_block.contains('"checkout": "25f52da"'), "addons manifest should not pin camera tracking to a git checkout for repo-local validation")
