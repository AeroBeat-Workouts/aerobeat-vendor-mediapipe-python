...
- `raw_tracking_frame.vendor_hand_tracking` surfaces the requested cadence knobs and constraints the tracker layer must honor upstream (`inference_interval_frames`, `bbox_recompute_interval_frames`, `max_stale_frames`, `reacquire_stable_frames`, backend/model availability, and API limits)
