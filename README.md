...
- `runtime.state_update_max_fps` — snapshot/state-debug write cap (default `15`)
- `runtime.preview_enabled` — enable/disable preview frame writes entirely (default `true`)
- `runtime.preview_max_fps` — preview image update cap, independent from `state_update_max_fps` (default `10`)
